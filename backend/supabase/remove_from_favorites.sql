create or replace function remove_from_favorites(user_id uuid, movie_id bigint)
returns boolean as $$
begin
  ...
  elsif not exists (select 1 from "Favorites list" where user_fk = user_id and movie_fk = movie_id) then
    return false;
